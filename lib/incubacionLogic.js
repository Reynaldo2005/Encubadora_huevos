...
const HORA_INICIO_VOLTEO = 6;
const HORA_FIN_VOLTEO = 22;
const VOLTEO_POR_DIA = 4;
const HORAS_ENTRE_VOLTEOS = (HORA_FIN_VOLTEO - HORA_INICIO_VOLTEO)/ VOLTEO_POR_DIA;
const TEMPERATURA_MAXIMA_ALERTA = 38.5; // °C
const HUMEDAD_MINIMA_ALERTA = 60; // %
...
export function calcularAlertaVolteo(
  ...
  if (horaActual < HORA_INICIO_VOLTEO || horaActual >= HORA_FIN_VOLTEO) return false;
  ...
  return horaSinVolteo >= HORAS_ENTRE_VOLTEOS
}

/** ¿La temperatura actual superó el máximo permitido? */
export function necesitaAlertaTemperaturaAlta(temperatura) {
  return temperatura > TEMPERATURA_MAXIMA_ALERTA;
}

/** ¿La humedad actual cayó por debajo del mínimo permitido? */
export function necesitaAlertaHumedadBaja(humedad) {
  return humedad < HUMEDAD_MINIMA_ALERTA;
}
